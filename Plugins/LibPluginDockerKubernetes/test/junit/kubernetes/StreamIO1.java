...
public class StreamIO1 {
	private static final Logger log = LogManager.getLogger(StreamIO1.class);
		...
		public void run() {
			while ( true) {
				try {
					...
					if ( chunck == null ) {
						System.out.println("Read a NULL chunk from socket. EOF? ERROR? Aborting reader loop.");
						break;
					}
					if ( ps != null) {
						ps.write(chunck);
			...
			System.out.println("SOCKET-READER loop terminated " + id );
		}
